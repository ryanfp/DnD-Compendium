...
1st turn. The storm buffets the area with a raging windstorm. No creature within the area can make ranged attacks or be the target of a ranged attack. A Small or smaller creature caught in the storm's area must make a Strength saving throw. On a failed save, the creature is swept up, thrown `2d6 x 10` feet in a random direction, and takes `d6` bludgeoning damage per ten feet thrown if it lands on a solid surface. A Medium creature must succeed on a Strength saving throw or be knocked prone. A Large or larger creature must succeed on a Strength saving throw or its speed drops to 0.

A Large flying creature that fails its saving throw is blown `1d6 x 5` feet in a random direction, Medium flying creatures are blown `1d6 x 10` feet, and Small flying creatures are blown `2d6 x 10` feet away.
